...
import { LineBalancingState, StyleData, Results, OperationData, MovementTimeData } from '@/lib/line-balancing';
...
// Simple standalone component with just the core functionality
export default function EnhancedLineBalancing2() {
  ...
  const [customMovementTimes, setCustomMovementTimes] = useState<MovementTimeData[]>([]);
  ...
  // Create state object - all properties directly referenced without destructuring
  const state: LineBalancingState = {
    uploadedFiles,
    setUploadedFiles,
    styles,
    setStyles,
    ...
    <div className="container mx-auto p-4 max-w-6xl min-h-screen">
      ...
      <Tabs defaultValue="setup">
        ...
        <TabsContent value="setup">
          <SetupTab state={state} />
        </TabsContent>

        <TabsContent value="calculations">
          <CalculationsTab state={state} />
        </TabsContent>

        <TabsContent value="visualization">
          <VisualizationTab state={state} />
        </TabsContent>
      </Tabs>
    </div>
  );
